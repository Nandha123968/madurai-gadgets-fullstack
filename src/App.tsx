...
export default function App() {
  ...
  const handleVariationFilesUpload = async (files: FileList) => {
    ...
    try {
      ...
      triggerToast(`Successfully uploaded and extracted ${newVariations.length} watch variations! 🎨`, "success");
    } catch (err) {
                ...
                <form
                  onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      if (response.ok) {
                        triggerToast(`Mass! "${newWatchName}" Database-la Add Aagiduchu! 🚀`, "success");
                ...
                >
                  ...
                  <div>
                    <ImageUploader
                      ...
                      label="Product Media (Image Uploader) 📸"
                  ...
                  <div className="space-y-4 text-xs text-gray-700">
                    ...
                    <div className="space-y-1.5">
                      <ImageUploader
                        ...
                        label="Custom Hero Banner Artwork 🎨"
              ...
              <div id="printable-invoice-card" className="bg-white border-l-[16px] border-amber-500 p-8 text-zinc-900 relative shadow-2xl space-y-6 flex flex-col justify-between font-sans">
                ...
                <div id="printable-invoice-actions" className="flex gap-3 justify-end pt-4 border-t border-gray-200 font-sans">
                  ...
                  {printingOrder && (
                    <a
                      href={`https://wa.me/${
                        (printingOrder.shipping.phone ? printingOrder.shipping.phone.replace(/\D/g, "") : "").length === 10
                          ? `91${printingOrder.shipping.phone.replace(/\D/g, "")}`
                          : (printingOrder.shipping.phone ? printingOrder.shipping.phone.replace(/\D/g, "") : "")
                      }?text=${encodeURIComponent(
                        `*MADURAI GADGETS 58 - OFFICIAL DIGITAL RECEIPT* 📜\n--------------------------------------\nVanakkam Machan! Here is your official order invoice verification receipt!\n\n*CUSTOMER DETAILS:*\n👤 *Name:* ${printingOrder.shipping.fullName}\n📞 *Phone:* ${printingOrder.shipping.phone || "Not specified"}\n📍 *Address:* ${printingOrder.shipping.address}, ${printingOrder.shipping.city} - ${printingOrder.shipping.zipCode}\n\n*ORDER DETAILS:*\n🆔 *Invoice ID:* ${printingOrder.id}\n📅 *Date:* ${printingOrder.date}\n💵 *Payment Status:* ${printingOrder.paymentStatus.toUpperCase()}\n\n--------------------------------------\n*ITEMS PURCHASED:*\n${printingOrder.items
                          .map((item) => `• ${item.name} (Qty: ${item.quantity}) - ₹${item.price.toLocaleString("en-IN")}`)
                          .join("\n")}\n\n💰 *GRAND TOTAL:* ₹${printingOrder.total.toLocaleString("en-IN")}\n--------------------------------------\nThank you for shopping with Madurai Gadgets 58! Wear Peak, Master Your Style! ☄️✨`
                      )}`}
                      target="_blank"
                      rel="noopener noreferrer"
